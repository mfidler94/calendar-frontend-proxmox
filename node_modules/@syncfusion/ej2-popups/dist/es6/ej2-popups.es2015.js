...
const FOCUSABLE_ELEMENTS_SELECTOR = 'input,select,textarea,button:enabled,a,[contenteditable="true"],[tabindex]';
/**
...
let Dialog = class Dialog extends Component {
    ...
    isActiveBtn(btnArray, isButton) {
        let buttonObj;
        if (!isNullOrUndefined(btnArray) && btnArray.length > 0) {
            for (let i = 0; i < btnArray.length; i++) {
                const btn = btnArray[i];
                if (btn) {
                    const isDisabled = isButton ? btn.element.disabled : btn.disabled;
                    const isHidden = isButton ? btn.element.hidden : btn.hidden;
    ...
    getFocusElement(target) {
        const items = target.querySelectorAll(FOCUSABLE_ELEMENTS_SELECTOR);
            ...
            if (this.isModal) {
                ...
                if ((isNullOrUndefined(this.btnObj)) && (!isNullOrUndefined(this.ftrTemplateContent))) {
                    ...
                    if (isNullOrUndefined(this.btnObj) && (!buttonObj || isNullOrUndefined(buttonObj.element))) {
                        const focusableItems = this.element.querySelectorAll(FOCUSABLE_ELEMENTS_SELECTOR);
                        if (focusableItems.length > 0) {
                            const validFocusableItems = [];
                            for (let i = 0; i < focusableItems.length; i++) {
                                if (!(focusableItems[i].hasAttribute('tabindex') &&
                                    focusableItems[i].getAttribute('tabindex') === '-1')) {
    ...
    disableElement(element, t) {
        const elementMatch = element ? element.matches : null;
        if (elementMatch) {
