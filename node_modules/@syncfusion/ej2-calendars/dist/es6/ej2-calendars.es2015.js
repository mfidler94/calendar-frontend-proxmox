...
let CalendarBase = class CalendarBase extends Component {
    ...
    keyActionHandle(e, value, multiSelection) {
        ...
        switch (e.action) {
            ...
            case 'tab':
                if (this.getModuleName() === 'datepicker' || this.getModuleName() === 'datetimepicker') {
                    const hasToday = this.showTodayButton && !isNullOrUndefined(this.todayElement);
                    const isLastFocusable = hasToday
    ...
    validateValues(multiSelection, values) {
        if (multiSelection && !isNullOrUndefined(values) && values.length > 0) {
            ...
            for (let skipIndex = 0; skipIndex < copyValues.length; skipIndex++) {
                ...
                const minDate = this.globalize.parseDate(minString, minFormatOption);
                const maxDate = this.globalize.parseDate(maxString, maxFormatOption);
...
class Islamic {
    ...
    islamicRenderDecade(e, value) {
        ...
        if (this.calendarInstance.locale === 'ar') {
            startHdrYr = Number(startHdrYr.toString().replace(/[٠١٢٣٤٥٦٧٨٩]/g, (d) => String.fromCharCode(d.charCodeAt(0) - 1632 + 48)));
            endHdrYr = Number(endHdrYr.toString().replace(/[٠١٢٣٤٥٦٧٨٩]/g, (d) => String.fromCharCode(d.charCodeAt(0) - 1632 + 48)));
        }
...
let DatePicker = class DatePicker extends Calendar {
    ...
    inputKeyActionHandle(e) {
        ...
        switch (e.action) {
            ...
            case 'enter':
                {
                    const inputUnchanged = this.inputElement.value === this.previousElementValue;
                    const valueUnchanged = this.value instanceof Date && this.previousDate instanceof Date
    ...
    strictModeUpdate(preventTimeZone) {
        let format;
        const pattern = /^y/;
    ...
    modelHeader() {
        ...
        if (this.fullScreenMode) {
            ...
            const modelTodayButton = this.showTodayButton
    ...
    updateFloatLabelOverflowWidth() {
        ...
        const label = container.querySelector('.e-float-text.e-label-bottom');
        let width = 0;
        ...
        if (label) {
            const labelWidth = (this.element.parentElement.offsetWidth) - width;
            if (labelWidth && !(this.cssClass && this.cssClass.split(' ').indexOf('e-outline') !== -1)) {
                label.style.width = `${labelWidth}px`;
            }
        }
    }
    clearFloatLabelOverflowWidth() {
        const container = this.inputWrapper.container;
        const label = container.querySelector('.e-float-text.e-label-top');
        if (label) {
...
let DateRangePicker = class DateRangePicker extends CalendarBase {
        ...
        if (!this.preventBlur) {
            ...
            if (!isNullOrUndefined(value) && value.trim() !== '') {
                ...
                if (range.length > 1) {
                    ...
                    let startDate = this.globalize.parseDate(this.getAmPmValue(range[0]).trim(), dateOptions);
                    ...
                    if (((isNullOrUndefined(startDate) || (typeof (startDate) === 'object' && isNaN(startDate.getTime()))
                        ...
                        && !isNullOrUndefined(this.inputFormatsString)) {
                        for (const format of this.inputFormatsString) {
                            const inputFormatOptions = { format: format, type: 'date', skeleton: 'yMd' };
                            if (isNullOrUndefined(startDate) || (typeof (startDate) === 'object' && isNaN(startDate.getTime()))) {
    ...
    selectRange(event, element) {
        ...
        const isCustomMin = this.min.getTime() !== new Date(1900, 0, 1).getTime();
        const isCustomMax = this.max.getTime() !== new Date(2099, 11, 31).getTime();
        if (this.currentView() === 'Year' && this.depth === 'Year') {
            const startMonth = new Date(this.min.getFullYear(), this.min.getMonth(), 1);
            if (!isNullOrUndefined(this.startValue) && isCustomMin && +this.startValue <= +startMonth) {
                this.startValue = this.min;
            }
            const endMonth = new Date(this.max.getFullYear(), this.max.getMonth() + 1, 0);
                ...
                this.trigger('open', eventArgs, (eventArgs) => {
                    ...
                    if (!this.openEventArgs.cancel) {
                        ...
                        if (!isNullOrUndefined(this.mobileRangePopupWrap) && !isNullOrUndefined(this.popupObj)
                            ...
                            const index = parseInt(this.popupObj.element.style.zIndex, 10) ?
...
class MaskedDateTime {
    ...
    normalizeTokenIndex(index, preferPrev = false) {
